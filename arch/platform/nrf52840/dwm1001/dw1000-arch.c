...
dw1000_spi_read(uint16 hdrlen, const uint8 *hdrbuf, uint32 len, uint8 *buf)
{
  ...
  memcpy(idatabuf, hdrbuf, hdrlen);
  
  status = critical_enter();
  ret_code_t ret = nrf_drv_spi_transfer(&spi, idatabuf, idatalength, itempbuf, idatalength);
  critical_exit(status);
  
  if(ret != NRF_SUCCESS) {
    LOG_ERR("SPI read failed: %lu\n", (uint32_t)ret);
    return -1;
  }
...
dw1000_spi_write(uint16 hdrlen, const uint8 *hdrbuf, uint32 len, const uint8 *buf)
{
  ...
  memcpy(idatabuf, hdrbuf, hdrlen);
  memcpy(idatabuf + hdrlen, buf, len);
  
  status = critical_enter();
  ret_code_t ret = nrf_drv_spi_transfer(&spi, idatabuf, idatalength, NULL, 0);
  critical_exit(status);
  
  if(ret != NRF_SUCCESS) {
    LOG_ERR("SPI write failed: %lu\n", (uint32_t)ret);
    return -1;
  }
